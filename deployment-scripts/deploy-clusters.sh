...
find . -type f -name "*" -print0 | while IFS= read -r -d '' file; do
    ...
    then
       ...
       curl -v POST $workspaceUrl/api/2.0/clusters/edit \
            -H "Authorization:Bearer $accessToken" \
            -H "Content-Type: application/json" \
            --data "$newJSON"
...
# Get a list of clusters so we know if we need to create or edit
clusterList=$(curl -v GET $workspaceUrl/api/2.0/clusters/list \
               -H "Authorization:Bearer $accessToken" \
               -H "Content-Type: application/json")
...
find . -type f -name "*" -print0 | while IFS= read -r -d '' file; do
    ...
    then
       ...
       curl -v POST $workspaceUrl/api/2.0/clusters/delete \
            -H "Authorization:Bearer $accessToken" \
            -H "Content-Type: application/json" \
            --data "$newJSON"
    fi     
